...
copy-pasteable starting <style> block:
    ...
    <style>
      :root {
        --bg: #ffffff; --fg: #1a1a1a; --muted: #666; --accent: #6d5dfc;
        --border: #e2e2e2; --danger: #c0392b; --success: #1f8a4c; --warn: #b8860b;
        --space-2: 8px; --space-3: 16px; --space-4: 24px; --space-6: 40px;
      }
      @media (prefers-color-scheme: dark) {
        :root { --bg: #16161a; --fg: #eee; --muted: #999; --border: #333; }
      }
      body {
        font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto,
          Helvetica, Arial, sans-serif;
        background: var(--bg); color: var(--fg); line-height: 1.55;
        max-width: 880px; margin: 0 auto; padding: var(--space-6) var(--space-4);
      }
      h2 { margin-top: var(--space-6); border-bottom: 1px solid var(--border);
        padding-bottom: var(--space-2); }
      .chip { display: inline-block; padding: 2px 10px; border-radius: 999px;
        font-size: 0.8em; font-weight: 600; }
      .chip-status-draft { background: #eee; color: #444; }
      .chip-status-in-review { background: #fff3cd; color: #7a5b00; }
      .chip-status-reviewed { background: #d4edda; color: #1f8a4c; }
      .chip-high { background: #f8d7da; color: var(--danger); }
      .chip-med { background: #fff3cd; color: var(--warn); }
      .chip-low { background: #d4edda; color: var(--success); }
      .decision-card { border: 1px solid var(--border); border-radius: 10px;
      ...
      .tag-recommended { color: var(--success); font-weight: 600; }
      .btn-queue, .btn-approve, .btn-request-changes {
        border: none; border-radius: 8px; padding: 8px 16px; font-weight: 600;
        cursor: pointer; font-size: 0.95em;
      }
      .btn-queue { background: var(--accent); color: #fff; }
      .btn-approve { background: var(--success); color: #fff; }
      .btn-request-changes { background: transparent; color: var(--danger);
      ...
      .arch-diagram .box { fill: none; stroke: var(--accent); stroke-width: 2; }
...
prompts, and a couple of other lavish-axi input patterns worth porting. Read
...
Add these classes to the shared style block (section 0):
    ...
    .item-x, .item-restore, .list-add-btn { cursor: pointer; border: 1px solid var(--border);
      background: transparent; border-radius: 6px; font-size: 0.85em; padding: 1px 8px; }
    .item-x { color: var(--danger); } .item-restore { color: var(--accent); }
...
--- 6.4 OTHER LAVISH INPUT PATTERNS — ported, and one worth adding ---
...
Fill-in text fields for open questions that DO need prose (not every open
question fits a decision card, and section 12 is exactly for those) are also
worth porting, as a per-question submit form so partial typing never queues
early:

    <form onsubmit="event.preventDefault();
      var v = this.answer.value.trim(); if (!v) return;
      window.brain && window.brain.queuePrompt({ tag: 'message',
        queueKey: 'open-q-region-replication', prompt: v });
      this.answer.value = '';">
      <input name="answer" type="text" placeholder="Answer this open question..." data-brain-action />
      <button type="submit" data-brain-action>Submit</button>
    </form>
...
- Inline CSS in one <style> block (section 0). System font stack, not a
  webfont — it must render identically with zero network access.
...
- Every decision must be answerable by clicking only — no decision card
  ...
  the questions you already know how to phrase as a shortlist.
