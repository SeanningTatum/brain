...
function renderExecution(execution) {
  ...
  if (!shots.length) {
    ...
    // Carousel payload for the shared lightbox — parallel to the rendered thumbs.
    const payload = shots.map((s) => ({ url: shotUrl(s.rel), caption: s.caption || s.rel || "" }));
    shots.forEach((s, idx) => {
      ...
      img.addEventListener("click", () => {
        ...
        if (window.BrainLightbox) window.BrainLightbox.open(payload, idx);
