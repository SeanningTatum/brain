...
import { ensurePlan, recordReviewRound, planContext, slugForFile, executionContext, brainCheck, featureExists, watchContext, addAnnotation, removeAnnotation, markAnnotationsSent } from "./brain-data.js";
...
const VALID_TAGS = ["element", "text", "message", "screenshot", "reference-image", "decision", "diagram-node", "list-edit"];
...
function normalizeTarget(tag, target) {
  ...
  if (tag === "reference-image") {
    // A pin dropped on a reference <img> embedded in the artifact. `selector`
    // is the cssPath locator the agent uses to find the img in the artifact
    // source (the src itself may be an unusable data: URI). x/y are pin
    // percentages (0-100), null when the annotation carries no pin.
    const clampPct = (v) => (Number.isFinite(Number(v)) ? Math.max(0, Math.min(100, Number(v))) : null);
    return {
      type: "reference-image",
      selector: capString(target.selector, 300),
      alt: capString(target.alt, 300),
      x: clampPct(target.x),
      y: clampPct(target.y),
      note: capString(target.note, 2000),
    };
...
// POST /watch/<feature>/annotate/delete?brain=... — remove a persisted pin.
// Mirrors handleWatchAnnotate exactly: same-origin guard + resolveWatch
// validation + JSON body -> removeAnnotation, 400 on throw (unknown id or
// corrupt annotations file).
async function handleWatchAnnotateDelete(req, res, url, m) {
  if (!isSameOrigin(req)) return sendJSON(res, 403, { error: "cross-origin request refused" });
  const resolved = resolveWatch(res, url, m);
  if (!resolved) return;
  let body;
  ...
  const { id } = body || {};
  try {
    removeAnnotation(resolved.brain, resolved.feature, id);
    sendJSON(res, 200, { ok: true });
  } catch (e) {
    sendJSON(res, 400, { error: e.message });
  }
...
// POST /watch/<feature>/annotate/send?brain=... — mark every unsent pin on
// this feature as sent (composer hand-off). Mirrors handleWatchAnnotateDelete:
// same-origin guard + resolveWatch validation + JSON body, but the body
// carries no fields the route needs — an empty/absent body is fine, same as
// every other POST here that goes through readJSONBody (missing body -> {}).
async function handleWatchAnnotateSend(req, res, url, m) {
  if (!isSameOrigin(req)) return sendJSON(res, 403, { error: "cross-origin request refused" });
  const resolved = resolveWatch(res, url, m);
  if (!resolved) return;
  try {
  ...
  try {
    const { sent } = markAnnotationsSent(resolved.brain, resolved.feature);
...
const ROUTES = [
  ...
  // All three patterns are $-anchored (distinct, non-overlapping), so match
  // order is not load-bearing here — kept in most-specific-path-first order
  // anyway per convention.
  { method: "POST", pattern: /^\/watch\/([^/]+)\/annotate\/delete$/, handler: (req, res, url, m) => handleWatchAnnotateDelete(req, res, url, m) },
  { method: "POST", pattern: /^\/watch\/([^/]+)\/annotate\/send$/, handler: (req, res, url, m) => handleWatchAnnotateSend(req, res, url, m) },
