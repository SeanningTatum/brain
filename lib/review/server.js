...
import { ensurePlan, recordReviewRound, planContext, slugForFile, executionContext, brainCheck } from "./brain-data.js";
...
function findAnchorLine(lines, anchor) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (collapseWs(lines[i]).toLowerCase().includes(needle)) return i + 1;
  }
  return null;
}
...
function handleContext(res, key) {
  ...
  if (session.feature) payload.execution = executionContext(session.brain, session.feature);
