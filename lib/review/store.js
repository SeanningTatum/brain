...
function openInternal({ file, brain, plan, feature }, force) {
  ...
  if (existing) {
    ...
    if (feature !== undefined) existing.feature = feature || null;
...
export function openSession({ file, brain, plan, feature }) {
  return openInternal({ file, brain, plan, feature }, false);
}

export function openSessionForce({ file, brain, plan, feature }) {
  return openInternal({ file, brain, plan, feature }, true);
}
...
// ---------------------------------------------------------------------------
// Layout audit warnings (v6.2 — Addendum v6). session.layout_warnings is
// replaced wholesale on every /api/layout POST; delivered_layout_keys is a
// capped "have we ever handed this warning to a poller" set (kind:selector),
// so server.js can mark a re-delivered still-active warning `persistent`.
// ---------------------------------------------------------------------------

export function setLayoutWarnings(key, warnings) {
  const state = loadState();
  const session = state.sessions[key];
  if (!session) return null;
  session.layout_warnings = Array.isArray(warnings) ? warnings : [];
  session.updated_at = new Date().toISOString();
  saveState(state);
  return session;
}

export function markLayoutKeysDelivered(key, keys) {
  const state = loadState();
  const session = state.sessions[key];
  if (!session) return null;
  const existing = Array.isArray(session.delivered_layout_keys) ? session.delivered_layout_keys : [];
  const merged = [...existing, ...(keys || [])];
  session.delivered_layout_keys = [...new Set(merged)].slice(-100);
  session.updated_at = new Date().toISOString();
  saveState(state);
  return session;
}
