...
(function () {
  ...
  var zoomBtn = null;
  var zoomBtnTarget = null; // the svg OR reference <img> the hover button points at
  var zoomBtnKind = "diagram"; // "diagram" (in-frame pan/zoom) | "image" (parent lightbox)
  var zoomBtnCursorEl = null; // reference <img> whose inline cursor we overrode (image kind only)
  var zoomBtnPrevCursor = ""; // its prior inline cursor, restored on hide/switch
  ...
  // ---- reference-image annotation (cross-frame pin-drop lightbox) ----
  //
  // Reference <img> embedded in the artifact (design references, screenshots
  // the plan points at) get the SAME hover "⤢" affordance as diagrams and, on
  // explore-mode click, open the shared full-viewport pin-drop lightbox
  // (window.BrainLightbox) — but that lightbox lives in the CHROME, not here,
  // so we can't open it directly: we postMessage the image up and the chrome
  // opens it. Annotate-mode clicks are unchanged (generic element annotation);
  // the hover button opens the lightbox in either mode.
  //
  // Which images qualify: reasonably-sized (both dims >= REF_IMG_MIN), not our
  // own UI, not a link's child (those are navigational). `data-brain-zoom`
  // force-opts-in regardless of size; `data-brain-no-zoom` force-opts-out.

  var REF_IMG_MIN = 80;

  function isReferenceImage(img) {
    if (!img || img.nodeType !== 1) return false;
    if (String(img.tagName).toLowerCase() !== "img") return false;
    if (typeof img.closest !== "function") return false;
    if (img.closest("[data-brain-ui]")) return false;
    if (img.hasAttribute("data-brain-no-zoom")) return false;
    if (img.hasAttribute("data-brain-zoom")) return true; // opt-in ignores size
    if (img.closest("a[href]")) return false; // linked image = navigation, not a reference
    var r = img.getBoundingClientRect();
    return r.width >= REF_IMG_MIN && r.height >= REF_IMG_MIN;
  }

  // <img> is a void element, so an event target inside one is always the img
  // itself — no descendant walk needed.
  function findReferenceImage(node) {
    var img = node && node.nodeType === 1 && String(node.tagName).toLowerCase() === "img" ? node : null;
    return img && isReferenceImage(img) ? img : null;
  }

  // Display caption for the lightbox: alt, else an enclosing <figure>'s caption.
  function referenceCaption(img) {
    var alt = (img.getAttribute("alt") || "").trim();
    if (alt) return alt.slice(0, TEXT_CAP);
    if (typeof img.closest === "function") {
      var fig = img.closest("figure");
      var cap = fig && fig.querySelector("figcaption");
      if (cap) return (cap.textContent || "").replace(/\s+/g, " ").trim().slice(0, TEXT_CAP);
    }
    return "";
  }

  // Hand the image off to the chrome's BrainLightbox. We send the RESOLVED
  // absolute src (currentSrc) so the chrome — a different document — loads the
  // exact same bytes, plus a cssPath selector as the durable locator the agent
  // uses to find the <img> in the artifact source (the src may be a giant data:
  // URI, useless as an anchor).
  function openReferenceImage(img) {
    send("brain:openReferenceImage", {
      url: img.currentSrc || img.src || "",
      caption: referenceCaption(img),
      alt: (img.getAttribute("alt") || "").trim().slice(0, TEXT_CAP),
      selector: cssPath(img)
    });
  }

  function restoreZoomCursor() {
    if (zoomBtnCursorEl) {
      zoomBtnCursorEl.style.cursor = zoomBtnPrevCursor;
      zoomBtnCursorEl = null;
      zoomBtnPrevCursor = "";
    }
  }
  ...
  function ensureZoomBtn() {
    ...
    zoomBtn.addEventListener("click", function (e) {
      e.preventDefault();
      e.stopPropagation();
      if (!zoomBtnTarget) return;
      if (zoomBtnKind === "image") openReferenceImage(zoomBtnTarget);
      else openLightbox(zoomBtnTarget);
    });
    (document.body || document.documentElement).appendChild(zoomBtn);
    return zoomBtn;
  }

  function showZoomBtn(target, kind) {
    var btn = ensureZoomBtn();
    var rect = target.getBoundingClientRect();
    if (!rect.width || !rect.height) return;
    kind = kind || "diagram";
    // Switching targets (e.g. moving off an image) must restore its cursor first.
    if (zoomBtnCursorEl && zoomBtnCursorEl !== target) restoreZoomCursor();
    zoomBtnTarget = target;
    zoomBtnKind = kind;
    btn.title = kind === "image" ? "Annotate image" : "Zoom diagram";
    // Reference images have no CSS zoom-in cursor rule (unlike diagrams), so set
    // it inline while hovered and restore it on hide. (Annotate mode's global
    // crosshair !important wins over this — harmless; the button still shows.)
    if (kind === "image" && zoomBtnCursorEl !== target) {
      zoomBtnCursorEl = target;
      zoomBtnPrevCursor = target.style.cursor;
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      var svg = findDiagramSvg(t);
      if (svg) {
        showZoomBtn(svg, "diagram");
        return;
      }
      var img = findReferenceImage(t);
      if (img) {
        showZoomBtn(img, "image");
        return;
      }
      hideZoomBtn();
    },
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      var img = findReferenceImage(e.target);
      if (img) {
        e.preventDefault();
        e.stopPropagation();
        openReferenceImage(img);
      }
    },
    true
  );
