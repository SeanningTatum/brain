...
// twice (guarded below).
//
// Delivery-agnostic annotation layer (Phase 3, extended Phase 4 — composer
// send step). The lightbox owns the UI — dropping pins, the note composer,
// rendering existing pins, the unsent-count send affordance — but knows
// nothing about how an annotation is persisted or sent. `opts`:
...
//       commits the pin. If the resolved value is an object carrying an `id`
//       (the server's persisted record), it is merged over the local fields
//       and THAT is what gets pushed into the working `annotations` array —
//       so a freshly committed pin is immediately deletable without a reload.
//       If omitted, the Annotate button is hidden entirely.
//   opts.annotations             [{ shot, x, y, note, superseded, sentAt }]
//       pre-existing annotations. Open ones render as numbered pins on their
//       shot; superseded ones are NOT drawn as pins (a "<n> resolved earlier"
//       count in the chrome instead). `sentAt` (ISO string | null) is read by
//       the delete flow below to decide whether deleting a pin should give
//       back an unsent-count slot.
//   opts.annotatedCta            { text, actionLabel, action } — the what-next
//       toast shown after a pin commits. `text` says where the note went and how
//       to continue; `actionLabel` (optional) puts a button on the toast whose
//       click runs `action({ close, send })` — `close` closes the lightbox,
//       `send` triggers the SAME internal send routine the topbar Send button
//       uses (only meaningful when opts.onSendFeedback is also given).
//       Without this opt a generic "Pin saved" toast shows, so each surface
//       should supply its own delivery-specific CTA.
//   opts.onDeleteAnnotation(annotation)  called when a reviewer clicks Delete
//       on a committed pin's hover note; annotation is the record from
//       `annotations` (needs an `id` to be deletable server-side). May return
//       a Promise: rejection shows an inline error under the note; resolution
//       removes the pin from the working `annotations` array and re-renders.
//       Omitted -> no Delete control on any pin (read-only, e.g. chrome.js).
//       Deleting a pin whose `sentAt` is null/absent (still unsent) also gives
//       back one slot in the unsent counter below (clamped at zero); deleting
//       an already-sent pin leaves the counter untouched.
//   opts.unsentCount              number — initial count of pins the reviewer
//       has drafted but not yet sent. Seeds the internal counter this open()
//       call tracks: +1 per pin committed via onAnnotate, -1 (clamped at 0)
//       per unsent pin removed via onDeleteAnnotation. Only meaningful
//       together with opts.onSendFeedback.
//   opts.onSendFeedback()         called when the reviewer sends the current
//       batch — via the topbar Send button, or an annotatedCta action calling
//       the `send` it's handed. May return a Promise: rejection re-enables the
//       button and toasts the error; resolution zeroes the unsent counter,
//       hides the button, and toasts "Sent — Claude will pick this up."
//       Omitted -> no Send button, no unsent-count tracking at all.
//
...
//       .bl-send-btn                "Send N pin(s) to Claude" (hidden at 0 /
//                                    without opts.onSendFeedback)
//     .bl-body
...
//             .bl-pin-note           hover note; child .bl-pin-delete when
//                                    opts.onDeleteAnnotation is set
//           .bl-notebox            pending-pin note composer
//       .bl-nav.bl-next             next arrow button
//     .bl-filmstrip
//       .bl-thumb (.active)         one per shot
//     .bl-toast                     shared toast: post-commit CTA, or the
//                                    send flow's result/error (text + optional
//                                    button) — one element, never both shown
//                                    at once, since a send disables the note
//                                    box and cannot race a pin-commit toast.
...
(function () {
  ...
  var CSS =
    ...
    "#brain-lightbox .bl-send-btn{flex:0 0 auto;font:inherit;font-size:12px;font-weight:600;" +
    "padding:5px 12px;border-radius:999px;border:1px solid #d97757;" +
    "background:#d97757;color:#fff;cursor:pointer;transition:opacity .12s;}" +
    "#brain-lightbox .bl-send-btn:hover:not(:disabled){opacity:0.85;}" +
    "#brain-lightbox .bl-send-btn:disabled{opacity:0.5;cursor:default;}" +
    ...
    // width:max-content — the note is absolutely positioned inside the ~22px
    // pin, so shrink-to-fit would resolve against the pin's width and wrap one
    // character per line without it.
    "#brain-lightbox .bl-pin-note{position:absolute;top:100%;left:50%;" +
    "transform:translate(-50%,8px);width:max-content;max-width:240px;text-align:left;" +
    "padding:8px 10px;border-radius:8px;" +
    "background:#141413;color:#faf9f5;font-size:12px;font-weight:400;line-height:1.4;" +
    // pointer-events:auto (not none) — a delete control lives inside this note,
    // and the note being hit-testable is also what keeps `.bl-pin:hover` (and
    // hence the note's own display:block below) true while the cursor moves
    // from the pin down onto the note itself: :hover cascades to ancestors of
    // whatever element is actually hit-tested under the cursor.
    "box-shadow:0 4px 20px rgba(0,0,0,0.5);pointer-events:auto;white-space:normal;" +
    ...
    "#brain-lightbox .bl-pin-note-actions{margin-top:6px;display:flex;justify-content:flex-end;}" +
    "#brain-lightbox .bl-pin-delete{font:inherit;font-size:11px;padding:2px 8px;border-radius:6px;" +
    "cursor:pointer;border:1px solid rgba(250,249,245,0.3);background:rgba(250,249,245,0.08);color:#faf9f5;}" +
    "#brain-lightbox .bl-pin-delete:hover{border-color:#e5896f;background:rgba(229,137,111,0.2);}" +
    "#brain-lightbox .bl-pin-delete:disabled{opacity:0.5;cursor:default;}" +
    "#brain-lightbox .bl-pin-note-err{color:#e5896f;font-size:11px;margin-top:4px;display:none;}" +
    "#brain-lightbox .bl-pin-note-err.show{display:block;}" +
    ...
    "#brain-lightbox .bl-toast{position:absolute;left:50%;bottom:96px;transform:translate(-50%,8px);" +
    "display:flex;align-items:center;gap:12px;max-width:min(560px,86vw);padding:10px 14px;" +
    "border-radius:10px;background:#141413;border:1px solid rgba(217,119,87,0.6);" +
    "box-shadow:0 8px 30px rgba(0,0,0,0.6);color:#faf9f5;font-size:13px;line-height:1.4;" +
    "opacity:0;pointer-events:none;transition:opacity .15s,transform .15s;z-index:20;}" +
    "#brain-lightbox .bl-toast.show{opacity:1;transform:translate(-50%,0);pointer-events:auto;}" +
    "#brain-lightbox .bl-toast button{flex:0 0 auto;font:inherit;font-size:12px;font-weight:600;" +
    "padding:5px 12px;border-radius:6px;cursor:pointer;border:1px solid #d97757;" +
    "background:#d97757;color:#fff;white-space:nowrap;}";
  ...
  var toastEl, toastTextEl, toastBtn, toastTimer;

  // send-flow state (Phase 4 — composer send step)
  var sendBtn;
  var unsentCount = 0;
  ...
  function build() {
    ...
    sendBtn.type = "button";
    sendBtn.className = "bl-send-btn";
    sendBtn.style.display = "none"; // shown per-open only when onSendFeedback given and unsentCount > 0
  ...
  // ---- annotation: post-commit CTA toast ------------------------------------
  ...
  // Shared by the post-pin-commit CTA (showToast) and the send flow
  // (triggerSend) — one toast element, one at a time. `cta.action`, when
  // given, is handed { close, send } so an annotatedCta can trigger the same
  // internal send routine the topbar button uses.
  function showToastWith(cta) {
    cta = cta && typeof cta === "object" ? cta : {};
    toastTextEl.textContent = cta.text || "Pin saved.";
    if (cta.actionLabel && typeof cta.action === "function") {
      toastBtn.textContent = cta.actionLabel;
      toastBtn.style.display = "";
      toastBtn.onclick = function (e) {
        e.stopPropagation();
        hideToast();
        cta.action({ close: close, send: triggerSend });
      };
    } else {
  ...
  function showToast() {
    showToastWith(opts.annotatedCta);
  }

  // ---- annotation: send-to-agent affordance (Phase 4) -----------------------

  function updateSendButton() {
    if (!sendBtn) return;
    if (!opts.onSendFeedback || unsentCount <= 0) {
      sendBtn.style.display = "none";
      return;
    }
    sendBtn.style.display = "";
    sendBtn.disabled = false;
    sendBtn.textContent = "Send " + unsentCount + " pin" + (unsentCount === 1 ? "" : "s") + " to Claude";
  }

  function triggerSend() {
    if (!opts.onSendFeedback || sendBtn.disabled) return;
    sendBtn.disabled = true;

    var result;
    try {
      result = opts.onSendFeedback();
    } catch (err) {
    ...
    Promise.resolve(result).then(
      function () {
        unsentCount = 0;
        updateSendButton();
        showToastWith({ text: "Sent — Claude will pick this up." });
      },
      function (err) {
        sendBtn.disabled = false;
        showToastWith({ text: err && err.message ? "Could not send: " + err.message : "Could not send — try again" });
      }
    );
  }
  ...
  function commitPending() {
    ...
    Promise.resolve(result).then(
      function (resolved) {
        ...
        // When onAnnotate resolved with the server's persisted record (an
        // object carrying an `id`), merge it over the local fields and push
        // THAT — otherwise a freshly pinned annotation has no id and can't be
        // deleted until the page reloads.
        var local = { shot: ann.shot, x: ann.x, y: ann.y, note: ann.note, superseded: false };
        var committed = resolved && typeof resolved === "object" && resolved.id ? Object.assign({}, local, resolved) : local;
        annotations.push(committed);
        cancelPendingPin();
        renderPins();
        if (opts.onSendFeedback) {
          unsentCount += 1;
          updateSendButton();
        }
        showToast();
  ...
  function makePin(a, num) {
    ...
    note.appendChild(document.createTextNode(a.note || ""));

    if (opts.onDeleteAnnotation) {
      var delErr = document.createElement("div");
      delErr.className = "bl-pin-note-err";

      var delRow = document.createElement("div");
      delRow.className = "bl-pin-note-actions";
      var delBtn = document.createElement("button");
      delBtn.type = "button";
      delBtn.className = "bl-pin-delete";
      delBtn.textContent = "Delete";
      delBtn.addEventListener("click", function (e) {
        // Must not toggle show-note (pin's own click handler) or close the
        // lightbox (the scrim's click handler) — stop right here.
        e.stopPropagation();
        ...
        var result;
        try {
          result = opts.onDeleteAnnotation(a);
        } catch (err) {
          result = Promise.reject(err);
        }
        ...
        Promise.resolve(result).then(
          function () {
            ...
            // An unsent pin (sentAt null/absent) frees up its unsent-count
            // slot; a pin already sent leaves the counter untouched.
            if (opts.onSendFeedback && !a.sentAt) {
              unsentCount = Math.max(0, unsentCount - 1);
              updateSendButton();
            }
            renderPins();
          },
          function (err) {
            ...
            delErr.textContent = err && err.message ? "Could not delete: " + err.message : "Could not delete — try again";
            delErr.classList.add("show");
          }
        );
      });
      delRow.appendChild(delBtn);
      note.appendChild(delRow);
      note.appendChild(delErr);
    }
  ...
  function open(list, startIndex, options) {
    ...
    unsentCount = typeof opts.unsentCount === "number" ? opts.unsentCount : 0;
    updateSendButton();
    setAnnotate(false);
    hideToast();
